...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/24 05:53:56 by gizawahr          #+#    #+#             */
/*   Updated: 2021/12/24 05:53:57 by gizawahr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!(par->img_id = mlx_new_image(par->mlx_id, par->res_w, par->res_h)))
		ft_quit("failed to create new img", par);
...
void	ft_game_starter(t_params *par)
{
	...
	ft_init_dir(par);
	if (!(par->window_id = mlx_new_window(par->mlx_id, par->res_w,
					par->res_h, "Cub3D")))
		ft_quit("Mlx window creation failed", par);
